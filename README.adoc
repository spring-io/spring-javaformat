...
=== Javadoc Offline Links Doclet
The `io.spring.javaformat:spring-javaformat-doclet` module can used if you generate javadoc using `-linkoffline`.
It allows `-linkoffline` locations to be expanded against source directories, allowing you to quickly configure links against `*-javadoc.jar` files.

To use the doclet, you'd first extract `package-list` and `element-list` files from javadoc jars.
	...
	<executions>
		<execution>
			...
			<configuration>
				...
				<outputDirectory>${project.build.directory}/javadoc-dependencies</outputDirectory>
...
You can then use the `io.spring.javaformat.doclet.OfflineLinksDoclet` configured with `-offlinelinks-source` directories and `-linkoffline` elements.
Each `-linkoffline` location can be a comma-separated list of names that will be expanded into complete file locations.
...
NOTE: You can't use the `<offlineLinks>` element because the location is not a direction.
You must use `<additionalOption>` with `linkoffline`.

Expansion works by replacing the `@name@` element in the `-offlinelinks-source` with each location name.

Unlike the standard Javadoc doclet, this doclet also checks that `{@link ...}` references are valid.
By default, the standard doclet silently converts links that it can't find into non-clickable text.

If you have packages that you don't have links for, you can use the `-offlinelinks-ignore-packages` option to ignore them.
